...
var useStyles = createMemoStyles.createMemoStyles({
  ...
  root: ({ theme, size, fullWidth, radius }) => {
    ...
    return __spreadProps(__spreadValues(__spreadValues({}, getFocusStyles.getFocusStyles(theme)), getFontStyles.getFontStyles(theme)), {
      fontSize,
      height,
      ...
      textDecoration: "none",
      padding: [0, getSizeValue.getSizeValue({ size, sizes: theme.spacing }) / 1.5],
  ...
  dot: ({ theme, color, size }) => {
    const dotSize = getSizeValue.getSizeValue({ size, sizes: dotSizes });
    return {
      backgroundColor: "transparent",
      color: theme.colorScheme === "dark" ? theme.colors.dark[0] : theme.colors.gray[7],
      borderColor: theme.colorScheme === "dark" ? theme.colors.dark[3] : theme.colors.gray[3],
      paddingLeft: getSizeValue.getSizeValue({ size, sizes: theme.spacing }) / 1.5 - dotSize / 2,
      "&::before": {
        content: '""',
        display: "block",
        width: dotSize,
