...
var useStyles = createMemoStyles.createMemoStyles({
  ...
  input: ({ size, radius, theme, reduceMotion, color }) => {
    const handleSize = getSizeValue.getSizeValue({ size, sizes: handleSizes });
    const borderRadius = getSizeValue.getSizeValue({ size: radius, sizes: theme.radius });
    return __spreadProps(__spreadValues({}, getFocusStyles.getFocusStyles(theme)), {
      WebkitTapHighlightColor: "transparent",
      position: "relative",
      borderRadius,
      backgroundColor: theme.colorScheme === "dark" ? theme.colors.dark[4] : theme.colors.gray[2],
      border: `1px solid ${theme.colorScheme === "dark" ? theme.colors.dark[4] : theme.colors.gray[3]}`,
      height: getSizeValue.getSizeValue({ size, sizes: switchHeight }),
      width: getSizeValue.getSizeValue({ size, sizes: switchWidth }),
      minWidth: getSizeValue.getSizeValue({ size, sizes: switchWidth }),
      padding: [0, 2],
      margin: 0,
      ...
      "&:checked": {
        ...
        "&::before": {
          transform: `translateX(${getSizeValue.getSizeValue({ size, sizes: switchWidth }) - getSizeValue.getSizeValue({ size, sizes: handleSizes }) - 6}px)`,
