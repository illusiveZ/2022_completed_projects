...
var useStyles = createMemoStyles.createMemoStyles({
  opened: {},
  root: ({ size, theme }) => __spreadProps(__spreadValues({}, getFocusStyles.getFocusStyles(theme)), {
    WebkitTapHighlightColor: "transparent",
    borderRadius: theme.radius.sm,
    width: getSizeValue.getSizeValue({ size, sizes }) + theme.spacing.xs,
    height: getSizeValue.getSizeValue({ size, sizes }) + theme.spacing.xs,
    padding: theme.spacing.xs / 2,
    backgroundColor: "transparent",
    border: 0,
    cursor: "pointer"
  }),
  burger: ({ size, color, reduceMotion }) => {
    const sizeValue = getSizeValue.getSizeValue({ size, sizes });
    return {
      position: "relative",
      userSelect: "none",
