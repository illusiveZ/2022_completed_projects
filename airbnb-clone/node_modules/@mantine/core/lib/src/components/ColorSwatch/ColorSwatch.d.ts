import React from 'react';
import { DefaultProps, MantineNumberSize } from '../../theme';
import { ComponentPassThrough } from '../../types';
export interface ColorSwatchProps extends DefaultProps {
    /** Swatch color value in any css valid format (hex, rgb, etc.) */
    color: string;
    /** Width, height and border-radius in px */
    size?: number;
    /** Swatch border-radius predefined from theme or number for px value */
    radius?: MantineNumberSize;
}
export declare function ColorSwatch<T extends React.ElementType = 'div', U = HTMLDivElement>({ component: Element, color, size, radius, className, themeOverride, children, ...others }: ComponentPassThrough<T, ColorSwatchProps> & {
    /** Get element ref */
    elementRef?: React.ForwardedRef<U>;
}): JSX.Element;
export declare namespace ColorSwatch {
    var displayName: string;
}
//# sourceMappingURL=ColorSwatch.d.ts.map